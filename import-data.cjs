...
async function importProducts() {
  ...
  try {
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      let description = sanitizeString(columns[4]); // ds_item
      
      // Se a descrição estiver vazia, usar o nome do produto da coluna 2
      if (!description || description.trim() === '') {
        description = sanitizeString(columns[2]); // Nome do produto
      }
      ...
      const family = familiaComercial || null; // Campo família do produto
